...
import com.bs.search.common.CustomOAuth2UserService;
import com.bs.search.common.KakaoOAuth2UserService;
import com.bs.search.common.Role;
   ...
   protected void configure(HttpSecurity http) throws Exception {
      http
              ...
              .and()
                 ...
                 .antMatchers("/search/**").hasRole(Role.USER.name()) // /api/v1/** 은 USER권한만 접근 가능
                 .anyRequest().authenticated() // anyRequest : 설정된 값들 이외 나머지 URL 나타냄, authenticated : 인증된 사용자
              .and()
                 .oauth2Login()
                 .loginPage("/login")
                 .defaultSuccessUrl("/search/main")
              .and()
                 .logout()
                 .logoutSuccessUrl("/")
              .and()
